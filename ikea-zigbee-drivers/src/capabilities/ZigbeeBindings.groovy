...
if (controlDevice != null && controlDevice != "----") {
    if (controlDevice == "0000") {
        ...
        Log.info "🛠️ Adding binding to device #${controlDevice} for clusters {{params.clusters}}"
        
        {{# params.clusters }}
        cmds += "he raw 0x${device.deviceNetworkId} 0x00 0x00 0x0021 {49 ${Utils.payload "${device.zigbeeId}"} ${Utils.payload "${device.endpointId}"} ${Utils.payload "{{ . }}"} 03 ${Utils.payload "${controlDevice}"} 01} {0x0000}" // Add device binding for cluster {{ . }}
        {{/ params.clusters }}
    }
...
if (controlGroup != null && controlGroup != "----") {
    if (controlGroup == "0000") {
        ...
        Log.info "🛠️ Adding binding to group ${controlGroup} for clusters {{params.clusters}}"
        {{# params.clusters }}
        cmds += "he raw 0x${device.deviceNetworkId} 0x00 0x00 0x0021 {49 ${Utils.payload "${device.zigbeeId}"} ${Utils.payload "${device.endpointId}"} ${Utils.payload "{{ . }}"} 01 ${Utils.payload "${controlGroup}"}} {0x0000}" // Add group binding for cluster {{ . }}
        {{/ params.clusters }}
    }
