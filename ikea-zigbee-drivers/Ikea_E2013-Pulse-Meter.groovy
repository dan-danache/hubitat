/**
 * IKEA Parasoll Pulse Meter (E2013)
 *
...
@Field static final String DRIVER_NAME = 'IKEA Parasoll Pulse Meter (E2013)'
...
metadata {
    definition(name:DRIVER_NAME, namespace:'dandanache', author:'Dan Danache', importUrl:'https://raw.githubusercontent.com/dan-danache/hubitat/master/ikea-zigbee-drivers/Ikea_E2013-Pulse-Meter.groovy') {
        ...
        // Attributes for devices.Ikea_E2013-Pulse-Meter
    ...
    // Commands for devices.Ikea_E2013-Pulse-Meter
    command 'setIndex', [[name:'Value*', type:'NUMBER']]
    command 'increment'
    ...
    preferences {
        input(
            ...
            <div style="min-height:55px; background:transparent url('https://dan-danache.github.io/hubitat/ikea-zigbee-drivers/img/Ikea_E2013.webp') no-repeat left center;background-size:auto 55px;padding-left:60px">
                IKEA Parasoll Pulse Meter (E2013) <small>v5.4.1</small><br>
                <small><div>
                • <a href="https://dan-danache.github.io/hubitat/ikea-zigbee-drivers/#parasoll-pulse-meter-e2013" target="_blank">device details</a><br>
        ...
        // Inputs for devices.Ikea_E2013-Pulse-Meter
        input(
            name:'indexUnit', type:'text', title:'Measuring unit', required:true,
            description:'Set the measuring unit for this index',
            defaultValue:'m³'
        )
        input(
            name:'indexIncrement', type:'number', title:'Increment value', required:true,
            description:'Increment index attribute with this amount on each pulse',
            defaultValue:0.01
        )
    }
}
...
List<String> updated(boolean auto = false) {
    ...
    // Preferences for devices.Ikea_E2013-Pulse-Meter
    if (indexUnit == null) {
        indexUnit = 'm³'
        device.updateSetting 'indexUnit', [value:indexUnit, type:'string']
    }
    log_info "🛠️ indexUnit = ${indexUnit}"
    
    if (indexIncrement == null) {
        indexIncrement = 0.01
        device.updateSetting 'indexIncrement', [value:indexIncrement, type:'number']
    }
    log_info "🛠️ indexIncrement = ${indexIncrement}"
...
// Implementation for devices.Ikea_E2013-Pulse-Meter
void setIndex(BigDecimal value) {
    log_info "🎬 Setting index to ${value} ..."
    String index = value.setScale(2, RoundingMode.HALF_UP).toPlainString()
    utils_sendEvent name:'index', value:index, unit:(indexUnit ?: 'm³'), descriptionText:"Index set to ${index}${indexUnit ?: 'm³'}", type:'digital'
}
void increment(String type = 'digital') {
    BigDecimal indexValue = (device.currentValue('index', true) ?: BigDecimal.ZERO) + new BigDecimal(indexIncrement ?: 0.01)
    String index = indexValue.setScale(2, RoundingMode.HALF_UP).toPlainString()
    utils_sendEvent name:'index', value:index, unit:(indexUnit ?: 'm³'), descriptionText:"Index is ${index}${indexUnit ?: 'm³'}", type:type
}
...
void parse(String description) {
    ...
    switch (msg) {
        ...
        // Events for devices.Ikea_E2013-Pulse-Meter
        // ===================================================================================================================
