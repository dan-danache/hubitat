 ...
 * @see https://zigbee.blakadder.com/Philips_RWL022.html
 */
...
metadata {
    definition(name:DRIVER_NAME, namespace:"dandanache", author:"Dan Danache", importUrl:"https://raw.githubusercontent.com/dan-danache/hubitat/master/ikea-zigbee-drivers/RWL022.groovy") {
...
def configure(auto = false) {
    ...
    // Configure Philips Hue Dimmer Switch (RWL022) specific Zigbee reporting
    // -- No reporting needed

    // Add Philips Hue Dimmer Switch (RWL022) specific Zigbee binds
    cmds += "zdo bind 0x${device.deviceNetworkId} 0x01 0x01 0xFC00 {${device.zigbeeId}} {}" // Hue Specific cluster

    // Remove Philips Hue Dimmer Switch (RWL022) specific Zigbee binds
    cmds += "he raw 0x${device.deviceNetworkId} 0x00 0x00 0x0022 {49 ${Utils.payload "${device.zigbeeId}"} ${Utils.payload "0x01"} ${Utils.payload "0x0005"} 03 ${Utils.payload "${location.hub.zigbeeEui}"} 01} {0x0000}" // Scenes cluster
    cmds += "he raw 0x${device.deviceNetworkId} 0x00 0x00 0x0022 {49 ${Utils.payload "${device.zigbeeId}"} ${Utils.payload "0x01"} ${Utils.payload "0x0006"} 03 ${Utils.payload "${location.hub.zigbeeEui}"} 01} {0x0000}" // On/Off cluster
    cmds += "he raw 0x${device.deviceNetworkId} 0x00 0x00 0x0022 {49 ${Utils.payload "${device.zigbeeId}"} ${Utils.payload "0x01"} ${Utils.payload "0x0008"} 03 ${Utils.payload "${location.hub.zigbeeEui}"} 01} {0x0000}" // Level Control cluster
...
def parse(String description) {
    ...
    switch (msg) {
        ...
        // ---------------------------------------------------------------------------------------------------------------
        // Handle Philips Hue Dimmer Switch (RWL022) specific Zigbee messages
        // ---------------------------------------------------------------------------------------------------------------
