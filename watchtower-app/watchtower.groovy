 ...
 *
 * @see https://github.com/dan-danache/hubitat/tree/main/watchtower-app
 */
...
@Field static final String APP_VERSION = '2.11.0'
...
@Field static final Map<String, Map<String, Object>> SUPPORTED_ATTRIBUTES = [
    ...
    index: [min:0, unit:'', minMax:false, probe: { device, state, events, begin, end -> calc5minIncrease(device, 'index', state) }],
]
...
void installed() {
    log.info "${app.label} has been installed"
    unschedule()
    schedule '0 0/5 * ? * * *', 'collectDeviceMetrics'
    schedule '0 4/5 * ? * * *', 'collectHubMetrics'
}
void updated() {
    log.info "${app.label} has been updated"
}
private void debug(String message) { if (logEnable) log.debug "${APP_NAME} ▸ ${message}" }
private void warn(String message) { log.warn "${APP_NAME} ▸ ${message}" }
private void error(String message) { log.error "${APP_NAME} ▸ ${message}" }
...
Map main() {
    boolean showInstall = app.installationState == 'INCOMPLETE'
    return dynamicPage(name:'main', install:true, uninstall:!showInstall) {
...
Map devices() {
    return dynamicPage(name:'devices', title:'Devices', install:false, uninstall:false) {
        ...
        devices.each {
            ...
            table += """
                <tr${highlightPosition == it[0] ? ' id="highlighted-row"' : ''}>
                    <td>${renderButton("viewDevice_${it[0]}", "${deviceName}<div class=\"text-600\">${it[2].join(', ')}</div>", 'Edit device configuration', 'view-btn dev-btn')}</td>
                    <td class="tbl-icon">${it[1]?.displayName ? renderButton("removeDevice_${it[0]}", '🗑️', 'Remove device configuration', 'view-btn dev-btn') : ''}</td>
                </tr>
            """
        }
        ...
        if (state.action == 'confirm') {
            DeviceWrapper device = app.getSetting("d.${state.removePosition}")
            section {
                paragraph """\
                    <div class="p-dialog-mask" style="display:flex; position: fixed; height: 100%; width: 100%; left: 0px; top: 0px; justify-content: center; align-items: center; pointer-events: none; z-index: 3203;" data-pc-section="mask">
                        <div class="p-dialog p-component" style="display: flex; flex-direction: column; pointer-events: auto;" role="dialog" data-pc-name="dialog" data-pc-section="root" data-pd-focustrap="true">
                            ...
                            <div id="dialog-body" class="p-dialog-content" data-pc-section="content">
                                Remove the configuration for <b>${device}</b> now?<br><br>
                                ${renderInfoBox("""
                                    The following data files will be automatically removed:
                                    <ul>
                                        <li>wt_${device.id}_5m.csv</li>
                                        <li>wt_${device.id}_1h.csv</li>
                                        <li>wt_${device.id}_1d.csv</li>
                                        <li>wt_${device.id}_1w.csv</li>
                                    </ul>
                                """)}
                            </div>
                            <div id="dialog-footer" class="p-dialog-footer" data-pc-section="footer">
                                ...
                                ${renderButton('removeDevice', 'Yes', 'Remove device configuration', 'mdl-button mdl-js-button mdl-button--accent mdl-button--raised', 'dialog-btn')}
                            </div>
                        </div>
                    </div>
                """
        ...
        if (state.position != null) {
            ...
            if (state.action == 'add') {
                section {
                    ...
                    if (device != null) {
                        List<String> allAttr = device.supportedAttributes*.name
            ...
            if (state.action == 'view') {
                section {
                    paragraph """
                        <b>${deviceName}</b>${device == null ? '' : "&nbsp;&nbsp;<a href=\"/device/edit/${device.id}\" target=\"_blank\" title=\"Open device page on a new tab\"><i class=\"pi pi-external-link\"></i></a>" }
                        ${device == null ? "<div class=\"text-600\">${HUB_ATTRIBUTES.join(', ')}</div>" : ''}
                    """
                    if (device != null) {
                        List<String> allAttr = device.supportedAttributes*.name
                            .findAll { SUPPORTED_ATTRIBUTES.containsKey(it) }
                    ...
                    paragraph """
                        Data files:
                        <ul>
                            <li><a href="/local/wt_${deviceId}_5m.csv" target="_blank" title="Download data file">wt_${deviceId}_5m.csv <i class="pi pi-external-link"></i></a></li>
                            <li><a href="/local/wt_${deviceId}_1h.csv" target="_blank" title="Download data file">wt_${deviceId}_1h.csv <i class="pi pi-external-link"></i></a></li>
                            <li><a href="/local/wt_${deviceId}_1d.csv" target="_blank" title="Download data file">wt_${deviceId}_1d.csv <i class="pi pi-external-link"></i></a></li>
                            <li><a href="/local/wt_${deviceId}_1w.csv" target="_blank" title="Download data file">wt_${deviceId}_1w.csv <i class="pi pi-external-link"></i></a></li>
                        </ul>
                    """
                }
            }
                ...
                paragraph """\
                    <div class="p-dialog-mask" style="display:${state.action == 'add' || state.action == 'view' ? 'flex' : 'none'}; position: fixed; height: 100%; width: 100%; left: 0px; top: 0px; justify-content: center; align-items: center; pointer-events: none; z-index: 3203;" data-pc-section="mask">
                        <div class="p-dialog p-component" style="min-width: 30vw; display: flex; flex-direction: column; pointer-events: auto;" role="dialog" data-pc-name="dialog" data-pc-section="root" data-pd-focustrap="true">
                            <div class="p-dialog-header" data-pc-section="header">
                                <span class="p-dialog-title" data-pc-section="title">${state.action == 'add' ? 'Add' : 'Edit'} device configuration</span>
                            </div>
                            <div id="dialog-body" class="p-dialog-content" data-pc-section="content"></div>
                            <div id="dialog-footer" class="p-dialog-footer" data-pc-section="footer">
                                ${ state.action == 'view' ? """
                                    ${renderButton('close', '✔&nbsp;&nbsp;Save', 'Save device configuration', 'mdl-button mdl-button--primary mdl-js-button mdl-button--raised', 'dialog-btn')}
                                """ : ''}
                                ${ state.action == 'add' ? """
                                    ${renderButton('cancel', '✖&nbsp;&nbsp;Cancel', 'Cancel add action', 'mdl-button mdl-js-button mdl-button--raised', 'dialog-btn')}
                                    ${renderButton(disableSaveButton, 'saveDevice', '✔&nbsp;&nbsp;Save', 'Add device configuration', 'mdl-button mdl-button--primary mdl-js-button mdl-button--raised', 'dialog-btn')}
                                """ : ''}
                            </div>
                        </div>
                    </div>
...
Map dashboards() {
    ...
    return dynamicPage(name:'dashboards', title:'Dashboards', install:false, uninstall:false) {
        ...
        if (dashboardList.size != 0) {
            ...
            dashboardList.each {
                table += """
                    <tr>
                        <td><a href="${makeDashboardURL(it)}" target="_blank">${it} <i class="pi pi-external-link"></i></a></td>
...
Map settings() {
    return dynamicPage(name:'settings', title:'Settings', install:false, uninstall:false) {
...
String renderCommonStyle() {
    ...
    <style>
        ...
        #app-table th { font-weight: bold; border-bottom: 2px rgba(0,0,0,.12) solid }
        #app-table td {
            text-align: left;
            padding: 0;
            white-space: nowrap;
...
List collectDeviceConfiguration() {
    List retVal = []
...
String makeDashboardURL(String dashboardName) {
    String prefix = useCloudLinks == true ? "${apiServerUrl}/${hubUID}/apps/${app.id}" : "${(fullLocalApiServerUrl =~ URL_PATTERN).findAll()[0][1]}"
    return "${prefix}/watchtower.html?name=${URLEncoder.encode(dashboardName, 'UTF-8')}&access_token=${state.accessToken}"
}
...
void appButtonHandler(String buttonName) {
    ...
    if (buttonName.startsWith('removeDevice_')) {
        state.removePosition = Integer.parseInt(buttonName.substring(13))
        state.action = 'confirm'
        return
    }
    ...
    if (buttonName == 'removeDevice') {
        ...
        // Remove CSV files
        deleteDataFiles(app.getSetting("d.${state.removePosition}"))

        // Cleanup settings
        app.removeSetting "d.${state.removePosition}"
        app.removeSetting "a.${state.removePosition}"

        // Reset lastPosition; to be calculated on next list render
        state.remove 'lastPosition'

        // Update action
        state.remove 'removePosition'
        state.action = 'list'
        return
    }
...
static String calc5minValue(DeviceWrapper device, String attribute, List<String> onValues, List<Event> events, long beginTimestamp, long endTimestamp) {
    ...
    for (int i = 0; i < events.size(); i++) {
        ...
        long eventTimeStamp = event.date.time
        if (eventTimeStamp < beginTimestamp || eventTimeStamp > endTimestamp) continue
...
static String calc5minIncrease(DeviceWrapper device, String attribute, Map state) {
    String currentValue = device.currentValue(attribute)
    if (currentValue == null) return ''
    String stateKey = String.format('v.%s_%s', device.id, attribute)
...
void collectDeviceMetrics() {
    ...
    if (logEnable) debug "Collecting device metrics: now=${now}, epoch=${now.toEpochSecond()}, dow=${now.dayOfWeek}, hour=${now.hour}, min=${now.minute}"
    collectDeviceConfiguration().each { conf ->
        DeviceWrapper device = conf[1]
        List<String> attrs = conf[2]
        String deviceId = device?.id ?: '0'

        try { update5MinData(now, device, attrs) } catch (Exception ex) { error "Failed to update5MinData for device #${deviceId}: ${ex.message}" }
        if (aggregateDisable == true) return
        
        if (now.minute != 0) return
        try { update1HourData(now, deviceId, attrs) } catch (Exception ex) { error "Failed to update1HourData for device #${deviceId}: ${ex.message}" }

        if (now.hour != 0) return
        try { update1DayData(now, deviceId, attrs) } catch (Exception ex) { error "Failed to update1DayData for device #${deviceId}: ${ex.message}" }

        if (now.dayOfWeek != DayOfWeek.MONDAY) return
        try { update1WeekData(now, deviceId, attrs) } catch (Exception ex) { error "Failed to update1WeekData for device #${deviceId}: ${ex.message}" }
    }
}
...
void update5MinData(ZonedDateTime now, DeviceWrapper device, List<String> attrs, int lookbackMinutes = 5) {
    ...
    attrs.each {
        String value = SUPPORTED_ATTRIBUTES[it].probe(device, state, events, beginDate.time, now.toInstant().toEpochMilli())
...
void appendDataRecord(String fileName, List<String> csvRecord, List<String> attrs, Long maxLines, boolean minMaxHeaders) {
    ...
    csvLines.addAll(loadDataLines(fileName, attrs, minMaxHeaders, maxLines - 1))
    csvLines.add("${csvRecord.join(',')}")
    uploadHubFile(fileName, csvLines.join('\n').replaceAll(',null', ',').bytes)
}

List<String> loadDataLines(String fileName, List<String> attrs, boolean minMaxHeaders, Long maxLines) {
    try {
        List<String> lines = new String(downloadHubFile(fileName), 'UTF-8').trim().split('\n')
        if (lines.size() < 2) return []

        List<String> oldAttrs = lines.get(0).split(',', -1).drop(1).findAll { !it.endsWith('_min') && !it.endsWith('_max') }
        List<String> dataLines = lines.drop(1).takeRight((int) maxLines)

        // Attributes did not change
        return attrs.containsAll(oldAttrs) && attrs.size() == oldAttrs.size() ? dataLines : patchDataLines(dataLines, oldAttrs, attrs, minMaxHeaders)
    } catch (NoSuchFileException ex) {
...
List<String> patchDataLines(List<String> oldLines, List<String> oldAttrs, List<String> newAttrs, boolean minMaxHeaders) {
    warn "Monitored attributes changed from ${oldAttrs} to ${newAttrs}. Migrating data file ..."
    int oldAttrsLen = oldAttrs.size()
    int newAttrsLen = newAttrs.size()

    List<String> newLines = []
    List<Integer> idxs = newAttrs.collect { oldAttrs.indexOf(it) }
    oldLines.each {
        String[] vals = it.split(',', -1)

        // Init newline and copy timestamp from/to position 0
        String[] newLine = new String[1 + newAttrs.size() * (minMaxHeaders ? 3 : 1)]
        newLine[0] = vals[0]

        // Process each attribute
        for (int i = 0; i < newAttrsLen; i++) {
            int idx = idxs.get(i)
            String val = '', min = '', max = ''

            // Copy value from its old position to the new position
            if (idx != -1) {
                val = vals[1 + idx]
                if (minMaxHeaders && vals.length > 1 + oldAttrsLen + idx) min = vals[1 + oldAttrsLen + idx]
                if (minMaxHeaders && vals.length > 1 + 2 * oldAttrsLen + idx) min = vals[1 + 2 * oldAttrsLen + idx]
            }
            newLine[1 + i] = val
            if (minMaxHeaders) {
                newLine[1 + newAttrsLen + i] = min
                newLine[1 + 2 * newAttrsLen + i] = max
            }
        }
...
String[] computeAveragesAndMinMax(String fileContents, List<String> attrs, Long onlyAfter) {
    if (fileContents == null) return new String[0]

    String[] fileLines = fileContents.trim().split('\n')
    if (fileLines.length < 2) return new String[0]
    ...
    // Divide sums by cnts to get the averages; exception is the 'energy' and 'index' attributes, that is an ever-increasing counter not a gauge
    String[] newRecord = new String[3 * attrsCount]
    for (int i = 0; i < attrsCount; i++) {
        if (attrs.get(i) == 'energy' || attrs.get(i) == 'index') {
...
void deleteDataFiles(DeviceWrapper device) {
    ...
    // Also cleanup app state entries used for counter attributes (e.g. energy, index)
...
Map getDashboardJsMapping() {
    if (logEnable) debug "Proxying watchtower.js to ${request.HOST} (${request.requestSource})"
    if (request.requestSource == 'cloud') return render(status:301, headers:[location:'https://dan-danache.github.io/hubitat/watchtower-app/watchtower.js'])
    return render(status:200, contentType:'text/javascript', data:new String(downloadHubFile('watchtower.js'), 'UTF-8'))
}

Map getDashboardCsvMapping() {
...
Map getAppManifestMapping() {
    ...
    return render(status:200, contentType:'application/manifest+json',
        data: """\
        {
            "id": "${UUID.nameUUIDFromBytes(params.name.bytes)}",
            "name": "${params.name}",
            "short_name": "${params.name}",
            "description": "View metrics for your smart devices.",
            "start_url": "${makeDashboardURL(params.name)}",
            "icons": [{
...
Map saveGridLayout(Map data) {
    state[data.stateEntryName] = data.json
    return render(status:200, contentType:'application/json', data:'{"status": true}')
}

Map getMonitoredDevicesMapping() {
...
Map getSupportedAttributesMapping() {
...
Map getCollectDeviceMetricsMapping() {
    ...
    collectDeviceConfiguration().each { conf ->
        ...
        List<String> attrs = conf[2]
        String deviceId = device?.id ?: '0'
        try { update5MinData(now, device, attrs, lookbackMinutes) } catch (Exception ex) { error "Failed to collect metrics for device #${deviceId}: ${ex.message}" }
    }
